...
def load_config() -> dict:
    ...
    return {
        ...
        'industry' : ['1162', '1164', '1166', '1260', '2775', '4993'],
        'region'   : ['00', '30'],
        'columns'  : [
            'bidNtceNo', 'bidNtceOrd', 'bidNtceNm',
            'bidBeginDt', 'bidClseDt', 'sucsfbidLwltRate',
            'rgstDt', 'industry', 'region'  # industry, region 추가
        ],
    }
...
def collect(config: dict) -> pd.DataFrame:
    ...
    frames = []
    total = len(config['industry']) * len(config['region'])
    done = 0

    for industry in config['industry']:
        for region in config['region']:
            done += 1
            i_label = INDUSTRY_LABEL.get(industry, industry)
            r_label = REGION_LABEL.get(region, region)
            print(f"[{done}/{total}] 업종: {i_label}({industry}) / 지역: {r_label}({region})")
            ...
            if items:
                ...
                df_part['industry'] = f"{i_label}({industry})"  # 예: 건물청소(1162)
                df_part['region']   = f"{r_label}({region})"    # 예: 전국(00)
